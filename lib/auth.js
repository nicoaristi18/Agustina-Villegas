...
// === CUPONES (compartidos con reservas) ===
// Storage: KV hash 'ag:coupons' con {code, type, value, uses_max, uses_current, expires, active}
// Tipos: 'percent' (X% off), 'fixed' (X UYU off), 'free' (100% off)
const COUPONS_KEY = 'ag:coupons';

export async function getCoupons() {
  const raw = await _upstash('HGETALL', COUPONS_KEY);
  if (!raw) return {};
  // Upstash devuelve array [k1, v1, k2, v2, ...] o {k1:v1, k2:v2} según versión
  const map = {};
  if (Array.isArray(raw)) {
    for (let i = 0; i < raw.length; i += 2) {
      try { map[raw[i]] = JSON.parse(raw[i + 1]); } catch { map[raw[i]] = raw[i + 1]; }
    }
  } else if (typeof raw === 'object') {
    for (const k of Object.keys(raw)) {
      try { map[k] = typeof raw[k] === 'string' ? JSON.parse(raw[k]) : raw[k]; } catch { map[k] = raw[k]; }
    }
  }
  return map;
}
...
export async function saveCoupon(coupon) {
  if (!coupon?.code) throw new Error('coupon.code requerido');
  await _upstash('HSET', COUPONS_KEY, coupon.code, JSON.stringify(coupon));
}

// Aplica un cupón a un precio base. No mutates el storage.
// Retorna { ok, error, finalPrice, discountAmount, coupon }
export function applyCoupon(coupon, basePrice) {
  if (!coupon) return { ok: false, error: 'Cupón no encontrado' };
  if (!coupon.active) return { ok: false, error: 'Cupón inactivo' };
  if (coupon.expires) {
    const exp = new Date(coupon.expires + 'T23:59:59');
    if (Date.now() > exp.getTime()) return { ok: false, error: 'Cupón vencido' };
  }
  const usesMax = Number(coupon.uses_max) || 0;
  const usesCur = Number(coupon.uses_current) || 0;
  if (usesMax > 0 && usesCur >= usesMax) return { ok: false, error: 'Cupón agotado (sin usos disponibles)' };
  ...
  if (coupon.type === 'percent') {
    ...
    discountAmount = Math.min(basePrice, Math.max(0, Number(coupon.value) || 0));
    finalPrice = basePrice - discountAmount;
  } else if (coupon.type === 'free') {
    discountAmount = basePrice;
    finalPrice = 0;
  } else {
    return { ok: false, error: 'Tipo de cupón desconocido' };
  }
  if (finalPrice < 0) finalPrice = 0;
  return { ok: true, finalPrice, discountAmount, coupon };
}

// Incrementa el contador de usos de un cupón (llamar solo después de una compra
// efectivamente concretada). Guarda el cupón actualizado.
export async function incrementCouponUse(code) {
  if (!code) return;
  const all = await getCoupons();
  const c = all[String(code).toUpperCase()];
  if (!c) return;
  c.uses_current = (Number(c.uses_current) || 0) + 1;
  await saveCoupon(c);
}
