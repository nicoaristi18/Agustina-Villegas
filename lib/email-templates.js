...
// Template para entrega de guía digital (Runner Principiantes, etc.)
export function guideDeliveryTemplate({ name, downloadUrl, productTitle }) {
  const firstName = (name || '').trim().split(' ')[0] || 'runner';
  const title = productTitle || 'Plan de Nutrición para Runners Principiantes';

  const body = `
    <h1 style="margin:0 0 12px 0;color:${BRAND_DARK};font-size:22px;font-weight:600;">¡Gracias por tu compra, ${escapeHtml(firstName)}! 🏃</h1>
    <p style="margin:0 0 16px 0;color:${BRAND_DARK};font-size:15px;line-height:1.6;">
      Ya podés acceder a tu <strong>${escapeHtml(title)}</strong>. Todo lo que necesitás para empezar a correr con la cabeza tranquila.
    </p>
    <p style="margin:0 0 24px 0;color:${BRAND_DARK};font-size:15px;line-height:1.6;">
      Hacé clic en el botón para descargar tu guía en PDF. El link es tuyo y no expira, guardalo bien.
    </p>
      ...
      <tr>
        <td align="center" style="padding:8px;">
          <table role="presentation" cellpadding="0" cellspacing="0" border="0">
            <tr>
              <td style="background:${BRAND_GOLD};border-radius:8px;">
                <a href="${escapeHtml(downloadUrl)}" style="display:inline-block;padding:16px 36px;color:#ffffff;font-size:16px;font-weight:600;text-decoration:none;letter-spacing:0.3px;">Descargar mi guía →</a>
              </td>
            </tr>
          </table>
        </td>
    ...
    <p style="margin:0 0 16px 0;color:${TEXT_MUTED};font-size:13px;line-height:1.6;text-align:center;">
      ...
      <a href="${escapeHtml(downloadUrl)}" style="color:${BRAND_GOLD};word-break:break-all;">${escapeHtml(downloadUrl)}</a>
    </p>

    <div style="margin:24px 0;padding:16px 20px;background:${BRAND_CREAM};border-radius:8px;border-left:3px solid ${BRAND_GOLD};">
      <p style="margin:0;color:${BRAND_DARK};font-size:13px;line-height:1.6;">
        💡 <strong>Tip:</strong> Leé la guía completa antes de tu próximo entrenamiento. Los capítulos sobre qué comer antes/después te van a ahorrar muchos dolores de estómago.
      </p>
    </div>

    <p style="margin:24px 0 0 0;color:${BRAND_DARK};font-size:14px;line-height:1.6;">
      Si tenés dudas sobre algo del contenido, escribime por WhatsApp.
    </p>
    <p style="margin:8px 0 0 0;color:${BRAND_DARK};font-size:14px;line-height:1.6;">
      ¡Buenos kilómetros! 🌿<br>
      <strong>Lic. Agustina Villegas</strong>
    </p>`;

  return {
    subject: `🏃 Tu guía "${title}" — descarga acá`,
    html: baseLayout({
      headerColor: BRAND_GOLD,
